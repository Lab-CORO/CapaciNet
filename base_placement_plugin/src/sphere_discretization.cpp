...
namespace sphere_discretization
{
    ...
    octomap::OcTree* SphereDiscretization::generateSphereTree(const octomap::point3d& origin, float radius, float resolution)
    {
        ...
        for (int i = 0; i < 360; i++)
        {
            for (int j = 0; j < 360; j++)
            {
                ...
                {
                    std::cout << "Error while inserting ray from " << origin << " to " << point_on_surface << std::endl;
                    // printf( "Error while inserting ray from %f to %f",  origin, point_on_surface);
                }
    ...
    void SphereDiscretization::make_sphere_poses(const geometry_msgs::msg::Point &origin, double r, std::vector< geometry_msgs::msg::Pose >& pose_Col)
    {
        ...
        static std::vector<geometry_msgs::msg::Point> position_vector(MAX_INDEX);
    ...
    double SphereDiscretization::r8_modp(double x, double y)
    {
        ...
        if (y = 0.0)
        {
            ...
            printf("R8-M0DP error: called with the y value of %f", y);
            exit(1);
        }
    ...
    void SphereDiscretization::convertPoseToVector(const geometry_msgs::msg::Pose& pose, std::vector< double > &data)
    {
        data.resize(7);
    ...
    void SphereDiscretization::convertVectorToPose(const std::vector< double >& data, geometry_msgs::msg::Pose &pose)
    {
    ...
    geometry_msgs::msg::Pose SphereDiscretization::findOptimalPose(const std::vector< geometry_msgs::msg::Pose >& poses, const octomap::point3d& origin)
    {
        geometry_msgs::msg::Pose optiPose;
    ...
    // void SphereDiscretization::createConeCloud(const geometry_msgs::msg::Pose& pose, const double opening_angle,
    ...
    void SphereDiscretization::poseToPoint(const geometry_msgs::msg::Pose& pose, octomap::point3d &point)
    {
    ...
    void SphereDiscretization::poseToEigenVector(const geometry_msgs::msg::Pose& pose, Eigen::VectorXd& vec)
    {
        vec << pose.position.x, pose.position.y, pose.position.z, pose.orientation.x, pose.orientation.y, pose.orientation.z,
                pose.orientation.w;
    }

    void SphereDiscretization::findOptimalPosebyPCA(const std::vector< geometry_msgs::msg::Pose >& probBasePoses,
    ...
    void SphereDiscretization::findOptimalPosebyAverage(const std::vector< geometry_msgs::msg::Pose > probBasePoses,
                                                        geometry_msgs::msg::Pose& final_base_pose)
    {
    ...
    void SphereDiscretization::associatePose(std::multimap< std::vector< double >, std::vector< double > >& baseTrnsCol,
                                             const std::vector< geometry_msgs::msg::Pose >& grasp_poses,
                                             ...
                                             const float resolution)
    {
        ...
        // First, we need to define the bounding box for the octree
        // Find min/max bounds of the point cloud
        pcl::PointXYZ min_pt, max_pt;
        if (cloud->empty())
        {
            RCLCPP_WARN(rclcpp::get_logger("sphere_discretization"),
                        "Point cloud is empty, cannot create octree");
            return;
        }
        ...
        RCLCPP_INFO(rclcpp::get_logger("sphere_discretization"),
                    "Point cloud bounds: [%.2f, %.2f, %.2f] to [%.2f, %.2f, %.2f]",
                    min_pt.x, min_pt.y, min_pt.z, max_pt.x, max_pt.y, max_pt.z);

        pcl::octree::OctreePointCloudSearch< pcl::PointXYZ > octree(resolution);
        octree.defineBoundingBox(min_pt.x, min_pt.y, min_pt.z, max_pt.x, max_pt.y, max_pt.z);
        ...
        for (int i = 0; i < spCenter.size(); i++)
        {
            ...
            // Check if search point is within the bounding box
            if (searchPoint.x < min_pt.x || searchPoint.x > max_pt.x ||
                searchPoint.y < min_pt.y || searchPoint.y > max_pt.y ||
                searchPoint.z < min_pt.z || searchPoint.z > max_pt.z)
            {
