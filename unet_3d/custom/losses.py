...
def gradient_loss_3d(pred, target):
    ...
    gz_pred = pred[:, :, :, :, 1:] - pred[:, :, :, :, :-1]

    gx_tgt = target[:, :, 1:, :, :] - target[:, :, :-1, :, :]
    gy_tgt = target[:, :, :, 1:, :] - target[:, :, :, :-1, :]
    gz_tgt = target[:, :, :, :, 1:] - target[:, :, :, :, :-1]

    return ((gx_pred - gx_tgt).abs().mean()
            + (gy_pred - gy_tgt).abs().mean()
            + (gz_pred - gz_tgt).abs().mean())


class BCEDiceGradientLoss(nn.Module):
    """BCEDiceLoss combined with a 3D gradient smoothness penalty.

    Penalises voxel-to-voxel discontinuities in the prediction that are not
    present in the target, encouraging spatially smooth outputs.
    Total loss = BCEDiceLoss(pred, target) + gradient_weight * gradient_loss_3d(pred, target)
    """

    def __init__(self, alpha=1.0, gradient_weight=0.1, **kwargs):
        super().__init__()
        self.bce_dice = BCEDiceLoss(alpha=alpha)
